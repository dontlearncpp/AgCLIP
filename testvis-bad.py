...
class TestData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = TTensor(image)
        print(fsc147_d_anno,im_id,dots.shape)
        global num
        num=im_id
        if im_id=='9808.jpg':
            aa=1
...
def main(args):
    ...
    for data_iter_step, (samples, gt_dots, text_descriptions) in enumerate(
        ...
        cv2.imwrite('vis-bad/'+data_loader_test.batch_sampler.sampler.data_source.img[data_iter_step], heatmap)
        ...
        if (cnt_err/gt_cnt > 0.1) & (gt_cnt>5):
            GOOD.append(data_loader_test.batch_sampler.sampler.data_source.img[data_iter_step])
            GOODNUM.append(pred_cnt)
            grou.append(gt_cnt)
    print("Averaged stats:", metric_logger)
    src_file = '/home/test/countx/CounTX-main-arg-up/FSC147_384_V2/images_384_VarV2/'
    dst_dir = '/home/test/countx/keshihua/img-bad'
    srcheat_dir = '/media/test/run/count/countx/CounTX-main-arg-4-conv-loss-auxloss/vis-bad/'
    heat_dir = '/home/test/countx/keshihua/heat-bad'
    for i in range(len(GOOD)):
        dst_file = os.path.join(dst_dir, str(GOOD[i][0:-4])+'-'+str(round(GOODNUM[i], 1))+'-'+str(grou[i])+'.jpg')
        shutil.copy(src_file+str(GOOD[i]),dst_file)
        shutil.copy2(srcheat_dir+str(GOOD[i]),heat_dir)


        print("good:", GOOD[i])
        print("gnum:", GOODNUM[i])
        print("grou",grou[i])
    log_stats = {
