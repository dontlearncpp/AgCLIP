...
from util.losses import ObjectNormalizedL2Loss
import torch
...
#--output_dir "./results" --img_dir "/media/test/run/count/countx/CounTX-main-arg/FSC147_384_V2/images_384_VarV2" --gt_dir "/media/test/run/count/countx/CounTX-main-arg/FSC147_384_V2/gt_density_map_adaptive_384_VarV2" --class_file "/media/test/run/count/countx/CounTX-main-arg/LearningToCountEverything-master/data/ImageClasses_FSC147.txt" --FSC147_anno_file "/media/test/run/count/countx/CounTX-main-arg/LearningToCountEverything-master/data/annotation_FSC147_384.json" --FSC147_D_anno_file "/media/test/run/count/countx/CounTX-main-arg/FSC-147-D.json" --data_split_file "/media/test/run/count/countx/CounTX-main-arg/LearningToCountEverything-master/data/Train_Test_Val_FSC_147.json"

#--output_dir "./results" --img_dir "/home/test/countx/CounTX-main-arg-up/FSC147_384_V2/images_384_VarV2" --gt_dir "/home/test/countx/CounTX-main-arg-up/FSC147_384_V2/gt_density_map_adaptive_384_VarV2" --class_file "/home/test/countx/CounTX-main-arg-up/LearningToCountEverything-master/data/ImageClasses_FSC147.txt" --FSC147_anno_file "/home/test/countx/CounTX-main-arg-up/LearningToCountEverything-master/data/annotation_FSC147_384.json" --FSC147_D_anno_file "/home/test/countx/CounTX-main-arg-up/FSC-147-D.json" --data_split_file "/home/test/countx/CounTX-main-arg-up/LearningToCountEverything-master/data/Train_Test_Val_FSC_147.json"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, gt_density, text_descriptions) in enumerate(
            ...
            with torch.cuda.amp.autocast():
                output,aux_out = model(samples, text_descriptions)
            ...
            pred_cnt = abs(torch.sum(output / 60).item())/ output.shape[0]
            gt_cnt = abs(torch.sum(gt_density / 60).item())/ output.shape[0]

            lossy = abs((gt_cnt-pred_cnt))
            # lossy = abs((gt_cnt-pred_cnt)/gt_cnt)
            # lossy = (1 - np.exp(-lossy*0.08))

            # ffl = FFL(loss_weight=1.0, alpha=1.0)  # initialize nn.Module class
            # fake = F.interpolate(output.unsqueeze(dim=1), size=(512, 512), mode='bilinear', align_corners=False)
            # real = F.interpolate(gt_density.unsqueeze(dim=1), size=(512, 512), mode='bilinear', align_corners=False)

            # fake = output.unsqueeze(dim=1).reshape(output.shape[0],1,256,256)
            # real = gt_density.unsqueeze(dim=1).reshape(output.shape[0],1,256,256)
            # lossffl = ffl(fake, real).cpu()  # calculate focal frequency loss
            # lossy = (1 - np.exp(-lossffl * 0.08))
            loss = (output - gt_density) ** 2
            loss = (loss * masks / (384 * 384)).sum() / output.shape[0]
            loss = loss + 0.001 * lossy

            aux_loss = sum([(aux_out[:,0,:,:]-gt_density) **2,(aux_out[:,1,:,:]-gt_density) **2])
            aux_loss = (aux_loss * masks / (384 * 384)).sum() / output.shape[0]
            ...
            for i in range(output.shape[0]):
                pred_cnt = abs(torch.sum(output[i] / 60).item())
